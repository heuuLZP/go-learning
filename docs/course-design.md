...
本课程采用 [`learning-method.md`](learning-method.md) 中的渐进披露、螺旋复现和提取练习。每关只增加一个主要新层次，同时重新调用至少一个旧概念，并明确哪些细节留到后续场景。

### 概念主线

| 主线 | 初次建立 | 逐层深入 | 综合使用 |
|---|---|---|---|
| HTTP 请求与响应 | 第 1 关追踪请求、handler 和响应 | 第 2–3 关加入方法、状态码、响应头和 JSON；第 4–7 关加入请求体、路径参数和错误响应 | 第 11–12 关串起数据库与 JSON 响应 |
| Todo 数据 | 第 3 关认识 `struct` 和列表 | 第 4–7 关在内存中创建、查找、更新和删除 | 第 9–12 关映射到 SQL 与持久化数据 |
| 状态与错误 | 第 2 关观察 `200` 和 `405` | 第 4–7 关依次使用 `201`、`404`、`400` 和 `204` | 第 11–12 关处理数据库错误和 `500` 边界 |
| 数据存储 | 第 4–7 关先使用内存 | 第 8 关建立 MySQL 环境，第 9–10 关完成 SQL CRUD | 第 11–12 关由 Go 接入并验证重启后仍存在 |

每关设计时额外标记：`旧锚点 / 主要新层次 / 明确延后 / 下一次复现位置`。通关只证明当前深度，不把“已经出现过”视为完全掌握。
...
- 每关回忆一个近期概念；每个阶段至少安排一次跨越 2–3 关的提取练习。
- 学习记录中的疑问用于调整下一关练习，但不随意改变阶段成果和概念主线。
